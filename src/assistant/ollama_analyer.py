...
async def ollama_vulnerability_check(flow: http.HTTPFlow, logger: Logger, model: str = "gemma3:1b") -> (float, str):
    ...
    log.threat = None
    log.threat_result = response
    if threat_match:
        log.threat = threat_match.group(1)
    if analysis_match:
        log.analysis = analysis_match.group(1)
